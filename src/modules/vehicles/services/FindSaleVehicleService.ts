...
export default class CreateVehicleRepository {
  ...
  public async execute(saleVehicle_id: string): Promise<VehicleSale> {
    const saleVehicle = await this.vehicleSaleRepository.findById(saleVehicle_id);

    if (!saleVehicle) {
      throw new AppError('Sale Vehicle is not found!!')
    }
